...
const Appoinment = () => {
  ...
  const { doctors ,currencySmpol} = useContext(AppContext)
  ...
  const fetchDocInfo = async () => {
    ...
    setDocInfo(docInfo || null); // Handle case where doctor is not found
    console.log(docInfo);
  }
          ...
          <p className='flex item-center gap-2 text-2xl font-medium text-gray-900'>
            ...
            <div>
              <p className='flex item-center gap-1 text-sm font-medium text-gray-900 mt-3'>
                About<img className='w-5'  src={assets.info_icon} alt=''/> </p>
              <p className='texxt-sm text-gray-500 max-w-[700px] mt-1'>{docInfo.about}</p>
            </div>
            <p className='text-gray-500 font-medium mt-4'>
              Appointment fee: <span className='text-gray-600'>{currencySmpol} {docInfo.fees}</span>
            </p>
        </div>
      </div>
        ...
        <div className='flex gap-3 item-center w-full overflow-x-scroll mt-4'>
          {
            docSlot.length && docSlot.map((item,index)=>(
              <div onClick={() =>setDocSlotIndex(index)}  className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white' : 'border border-grey-200'}`}  key={index}>
